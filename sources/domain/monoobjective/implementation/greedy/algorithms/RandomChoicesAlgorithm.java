 ...
 */
public class RandomChoicesAlgorithm extends GreedyAlgorithm {

    private Random rand;

    public RandomChoicesAlgorithm(Random rand, ProblemInstance instance, EvaluationFunction<Double, MatrixSolution> fx) {
        super(instance, fx);
        this.rand = rand;
    }
    ...
    public MatrixSolution run() {
        ...
        for (Request request : requests) {//para cada peticion...
            ...
            Collections.shuffle(request.vCUs, rand);
            isValid = true;
            for (VirtualNode vCU : request.vCUs) {//asignar CUs fisicos
                vCU.indxNode = -1;
                Collections.shuffle(instance.CUs, rand);
            ...
            if (!isValid) {//no se asignaron todas las CUs virtuales, liberar las asignadas..
                ...
                Collections.shuffle(request.vDUs, rand);
                for (VirtualNode vDU : request.vDUs) {//asignar DUs fisicos para cada DU virtual
                    vDU.indxNode = -1;
                    Collections.shuffle(instance.DUs, rand);
                        ...
                        if (!tDUs.contains(DU)) {
                            for (int vcu : vDU.nears) {//por cada enlace del vDU
                                ...
                                if (ProblemInstance.validateAssignament(vDU, DU) && instance.mPaths[DU.nodePosition][request.vNodes[vcu].indxNode] != null && !instance.mPaths[DU.nodePosition][request.vNodes[vcu].indxNode].isEmpty()) {
                                    ...
                                    Collections.shuffle(paths, rand);//validar la posible ruta y elegir el mejor split
